...
- **Sweep phase: DONE 2026-06-12** (one bounded gap-hunt agent over the post-fix tree, 11 hypotheses probed). Yield: **1 MEDIUM regression introduced by batch A** — the sitemap's `last_modified_at` fallback used `item.data.last_modified_at`, which Jekyll empties by key promotion exactly like `date`, making the branch dead (latent: no content used it yet) — fixed + probe-proven; **1 LOW hardening** — digit-boundary lookarounds on the FR-phone pattern so the widened full-tree privacy scan can't false-positive on digit runs in future minified JS/sourcemaps — applied; 1 LOW note — the `unless defined?(SITE)` guard doesn't silence the spec-constant collision warning (cosmetic; the consolidation remains an appendix backlog item). The methodology is now fully executed.
